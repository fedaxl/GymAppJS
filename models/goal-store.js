...
const goalStore = {
  ...
  goalStatus(goals, assessments) {
    ...
    for (i = 0; i < goals.length; i++) {
      ...
      if (isPast) {
        ...
        lastAssessmentBeforeGoalDate = this.findLastAssessmentPriorToGoalDate(
          goals[i],
          assessments,
          startweight
        );
        // use the goal details and the closest assessment details to check if the goal was achieved or missed
        isAchieved = this.checkIfGoalHasBeenAchieved(
          goals[i],
          lastAssessmentBeforeGoalDate
        );
        if (isAchieved) {
  ...
  findLastAssessmentPriorToGoalDate(goal, assessments, memberweight) {
    logger.debug(
      "Find the closest assessment recorded on or prior to the goal date"
    );
    ...
    for (i = assessments.length; i > 0; i--) {
      ...
      let stringAssessmentDateOnly = new Date(
        assessmentDate
      ).toLocaleDateString("ie-IE");
